...
test "Camera init sets target, right and up" {
    ...
    const expectedForward = zml.Vec3{ 1, 0, 0 };
    ...
    const expectedRight = zml.Vec3{ 0, 0, 1 };
    ...
    const expectedUp = zml.Vec3{ 0, -1, 0 };

    try expect(zml.vec3.approxEq(cam.target, expectedTarget, tol));
    try expect(zml.vec3.approxEq(cam.right, expectedRight, tol));
    try expect(zml.vec3.approxEq(cam.up, expectedUp, tol));
}
...
test "Camera rotate clamps pitch and wraps roll" {
    ...
    const forward = zml.forwardFromSphere(cam.yaw, cam.pitch);
...
test "Camera move updates pos and target" {
    ...
    const expectedPos = zml.Vec3{ -2, -4, 6 };
    try std.testing.expect(zml.vec3.approxEq(cam.pos, expectedPos, tol));
    // Target = pos + forward = (-2, -4, 6) + (0,0,1) = (-2, -4, 7)
    const expectedTarget = expectedPos + zml.Vec3{ 0, 0, 1 };
    try std.testing.expect(zml.vec3.approxEq(cam.target, expectedTarget, tol));
}
